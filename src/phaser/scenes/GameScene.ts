...
export class GameScene extends Phaser.Scene {
  ...
  private placeBomb(player: Player): void {
    ...
    const eggShine = this.add.ellipse(-5, -8, this.CELL_SIZE * 0.2, this.CELL_SIZE * 0.25, 0xFFFFFF, 0.6);
    ...
    directions.forEach(({ dx, dy }) => {
      for (let i = 0; i <= range; i++) {
        ...
        this.blocks.getChildren().forEach((block: any) => {
          if (Math.abs(block.x - cellX) < 10 && Math.abs(block.y - cellY) < 10) {
            // Destroy animation
            this.tweens.add({
              targets: block,
              alpha: 0,
  ...
  private spawnRandomPowerup(): void {
    ...
    while (!validPosition && attempts < maxAttempts) {
      ...
      this.indestructibleBlocks.getChildren().forEach((block: any) => {
        if (Math.abs(block.x - cellX) < 10 && Math.abs(block.y - cellY) < 10) {
          hasBlock = true;
        }
      });
      ...
      this.powerups.getChildren().forEach((powerup: any) => {
        if (Math.abs(powerup.x - cellX) < 10 && Math.abs(powerup.y - cellY) < 10) {
          hasPowerup = true;
        }
      });
      ...
      this.players.forEach((player) => {
        const sprite = player.getSprite();
        if (Math.abs(sprite.x - cellX) < this.CELL_SIZE && Math.abs(sprite.y - cellY) < this.CELL_SIZE) {
          hasPlayer = true;
        }
      });
    ...
    if (validPosition) {
      ...
      const powerup = this.add.circle(cellX, cellY, this.CELL_SIZE * 0.3, colors[type as keyof typeof colors]);
